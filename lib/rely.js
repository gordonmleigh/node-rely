...
module.exports = function(_options) {
  ...
  function rely(name, value) {
    if (value === undefined) {
      ...
      if (typeof dep === 'string') {
        ...
        return rely.autoRequire(dep, name);
   ...
   * via require().
   */
  rely.autoRequire = function rely_autoRequire(module, name) {
    // allows overriding of dependencies before call to this function
    var path = rely.dependencies['path'] || require('path');
    // if name is a path, expand the path
    if (module.indexOf('/') >= 0) { module = path.resolve(module); }
    // default for name override
    if (name == null) { name = module; }
    // now load the module
    var impl = rely.require(module);

    if (typeof impl === 'function') {
      if (impl.$rely !== undefined) {
        impl = rely.injectModule(impl);
      }
    }

    return rely(name, impl);
  };


  /**
   * Calls the module function, injecting dependencies.
   */
  rely.injectModule = function rely_injectModule(module) {
    if (!(module instanceof Function)) { throw new TypeError('rely: "module" must be a function'); }
