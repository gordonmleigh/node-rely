...
Module.prototype.get = function rely_module_get(async) {
  var self = this,
      deps = self.require(self.dependencies, async),
      def,
  ...
  if (self.loaded) {
    if (async) {
      return q(self.exports).nodeify(async);
    } else {
      return self.exports;
    }
  }
  // function to run after loading dependencies
  var next = function (deps) {
...
Rely.prototype.require = function rely_require(dependency, cwd, async) {
  ...
  if (Array.isArray(dependency)) {
    if (async) {
      ...
      return q.all(promise).nodeify(async);
    } else {
  ...
  // load the file
  if (async) {
    promise = self.loadFile(filename, dependency, true);
    return promise.then(function (def) {
      return def.exports;
    }).nodeify(async);
  } else {
...
Rely.prototype.loadFile = function rely_loadFile(filename, id, async) {
  ...
  if(async) {
    return q.nfcall(fs.readFile, filename, 'utf8').then(next).nodeify(async);
  } else {
